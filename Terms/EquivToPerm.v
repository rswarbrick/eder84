...
Section sequiv_means_perm.
  ...
  Lemma tau_sigma
    : fin_subst_ext_eq
        L (fin_subst_comp L decV decF
                          (var_subst_to_fin_subst L decV decF tau)
                          sigma)
        sigma'.
  Proof.
    intro v; autorewrite with fin_subst var_subst.
...
(** At this point, we have a constructive proof of Lemma 2.10 from
    Eder's paper. We have defined [tau] and [tau'] as in the paper and
    the factorings are [tau_sigma] and [tau'_sigma']. The fact that
    the two maps are inverse permutations is [ext_inverse_tau]. *)

Arguments tau_sigma {L} decV decF {sigma sigma'} equivH.
Arguments tau'_sigma' {L} decV decF {sigma sigma'} equivH.
Arguments ext_inverse_tau {L} decV {sigma sigma'} equivH.
