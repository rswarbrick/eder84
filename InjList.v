...
Lemma inj_on_list_length {A B : Type} {f : A -> B} (aa : list A) (bb : list B)
...
Proof.
  intro decB.
  revert bb.
  induction aa as [ | a aa IH ]; auto using le_0_n.
  intros bb distaH distbH lmH injH.
  destruct distaH as [ norepaH distaH ].
  specialize (IH (remove decB (f a) bb)
                 distaH
                 (distinct_remove decB (f a) distbH)
                 (is_list_map_uncons decB lmH norepaH injH)
                 (inj_on_list_uncons injH)).
  assert (In (f a) bb) as inbH.
  - unfold is_list_map in lmH; apply (lmH a (in_eq a aa)).
  - rewrite (length_distinct_remove_in decB distbH inbH).
